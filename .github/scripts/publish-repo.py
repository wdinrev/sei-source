...
ICON_BASE_URL = "https://cdn.jsdelivr.net/gh/wdinrev/sei-source@main"
...
remote_index_file = REPO_DIR.joinpath("index.json")
if remote_index_file.exists():
    with remote_index_file.open() as f:
...
for ext in final_extensions:
    apk_name = ext.resources.apkUrl.split("/")[-1] if ext.resources.apkUrl else ""
    pkg_parts = ext.packageName.split(".")
    lang = pkg_parts[4] if len(pkg_parts) > 4 else "id"
    min_json_data.append({
